...
public class VisitorGame extends GameEngine {
    private final int SCREEN_WIDTH = 1920;  // 1920
    private final int SCREEN_HEIGHT = 1080; // 1080
    private final double FOV = Math.toRadians(66);
    ...
    public void init() {
        ...
        player = new Player(7.25, 2.5, 0.0);
        mapManager = new MapManager();
        ...
        Start = loadAudio("Day.wav");
        if (Start != null) {
            // 启动循环播放，设置音量（-10dB 是适中的音量，可根据需求调整）
            startAudioLoop(Start, -10);
            System.out.println("BGM 播放中...");
        } else {
            System.out.println("BGM 加载失败，请检查文件路径！");
        }
    ...
    public void paintComponent() {
        // 清屏
        changeColor(Color.BLACK);
        drawSolidRectangle(0, 0, virtualWidth, virtualHeight);
